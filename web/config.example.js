// Copy this file to config.js and fill in real values.
// config.js is gitignored — it never gets committed or pushed to GitHub.
window.APP_CONFIG = {
  password: 'your-dashboard-password',
  urls: {
    giderler: 'https://docs.google.com/spreadsheets/d/e/.../pub?gid=...&single=true&output=csv',
    gelirler: 'https://docs.google.com/spreadsheets/d/e/.../pub?gid=...&single=true&output=csv',
    giderlerMutfak: 'https://docs.google.com/spreadsheets/d/e/.../pub?gid=...&single=true&output=csv',
    sahipMutfak: 'https://docs.google.com/spreadsheets/d/e/.../pub?gid=...&single=true&output=csv',
    sahipKahvehane: 'https://docs.google.com/spreadsheets/d/e/.../pub?gid=...&single=true&output=csv',
  }
};
